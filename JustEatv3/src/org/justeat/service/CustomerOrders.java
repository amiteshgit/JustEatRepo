...
public class CustomerOrders {
	...
	public ResultSet getOrderList(int userID) {
		...
		try {
			 ...
			 */
			queryString = "select order_id, to_char(order_time,'hh24:mi:ss') from justeat_orders where user_id="
					+ userID + " order by order_id desc";
	...
	public ResultSet getVendorList(int orderID) {
		...
		try {
			 ...
			 */
			queryString1 = "select jv.VENDOR_NAME,js.status_meaning,jvo.VENDOR_ORDER_ID,to_char(jvo.expected_time,'hh24:mi:ss') "
					+ " from JUSTEAT_VENDOR_ORDERS jvo,JUSTEAT_VENDORS jv, justeat_status js "
					+ " where jv.vendor_id=jvo.vendor_id " + " and jvo.status_id=js.STATUS_ID " + " and jvo.order_id="
					+ orderID;
	...
	public ResultSet getItemDetails(int vendorOrderID) {
		...
		try {
			...
			queryString2 = "select jm.item_name, jod.quantity, jod.amount"
					...
					+ " where jvo.VENDOR_ORDER_ID=jod.VENDOR_ORDER_ID" + " and jod.item_id=jm.ITEM_ID "
